...
const RATIO_COLORS: Record<string, string> = {
  'ratio-simplify':   'from-purple-500 to-violet-600',
  'ratio-equivalent': 'from-indigo-500 to-purple-600',
  'ratio-missing':    'from-violet-500 to-fuchsia-600',
  'ratio-share':      'from-fuchsia-500 to-pink-600',
  'ratio-word':       'from-purple-600 to-indigo-700',
};

const RATIO_BG: Record<string, string> = {
  'ratio-simplify':   'bg-purple-50 dark:bg-purple-950/60',
  'ratio-equivalent': 'bg-indigo-50 dark:bg-indigo-950/60',
  'ratio-missing':    'bg-violet-50 dark:bg-violet-950/60',
  'ratio-share':      'bg-fuchsia-50 dark:bg-fuchsia-950/60',
  'ratio-word':       'bg-purple-50 dark:bg-purple-950/60',
};

const TYPE_LABELS: Record<string, string> = {
  'ratio-simplify':   'Simplify Ratio',
  'ratio-equivalent': 'Equivalent Ratio',
  'ratio-missing':    'Missing Term',
  'ratio-share':      'Share Quantity',
  'ratio-word':       'Word Problem',
};

const RatioProportionCard = observer(() => {
  const {
    currentQuestion,
    wrongAnswers,
    ...
    if (showSuccess) {
      ...
      successTimerRef.current = setTimeout(() => {
        if (appStore.quizComplete) {
          ...
          const newQ = generateRatioProportionQuestion(difficulty);
  ...
  const gradientClass = RATIO_COLORS[type] || 'from-purple-500 to-violet-600';
  const bgClass       = RATIO_BG[type]     || 'bg-purple-50 dark:bg-purple-950/60';
  const typeLabel     = TYPE_LABELS[type]  || 'Ratio & Proportion';
  const accuracy =
  ...
  const handleSkip = () => {
    appStore.skipQuestion(generateRatioProportionQuestion(difficulty));
  };
      ...
      <div className={`w-full rounded-3xl shadow-2xl overflow-hidden border-4 border-white dark:border-gray-700 ${bgClass}`}>
        ...
        <div className="px-8 py-10 flex flex-col items-center gap-8">
                ...
                <button
                  ...
                  className={`
                    ...
                    ${isWrong
                      ...
                      : 'bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:scale-105 hover:shadow-lg hover:border-purple-300 dark:hover:border-purple-500 active:scale-95 cursor-pointer shadow-md'
                    }
                  `}
                >
                  <span
                    className={`text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center
                    ${isWrong ? 'bg-red-300 dark:bg-red-700 text-white' : isHint ? 'bg-gray-300 dark:bg-gray-600 text-white' : 'bg-purple-100 dark:bg-purple-900 text-purple-600 dark:text-purple-300'}`}
                  >
                    {labels[idx]}
                  </span>
          ...
          <button
            ...
            className="flex items-center gap-2 text-gray-400 dark:text-gray-500 hover:text-purple-500 dark:hover:text-purple-400 text-base font-semibold transition-colors disabled:opacity-40 mt-2"
          >
...
export default RatioProportionCard;
