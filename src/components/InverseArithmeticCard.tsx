...
const InverseArithmeticCard = observer(() => {
  const {
    currentInverseQuestion,
    wrongAnswerStrings,
    autoEliminatedStrings,
    showSuccess,
    score,
  ...
  }, [currentInverseQuestion?.id, showSuccess]);

  // Auto-eliminate a wrong option every 15 seconds
  useEffect(() => {
    if (showSuccess || quizComplete) return;
    const interval = setInterval(() => {
      if (appStore.showSuccess || appStore.quizComplete || !appStore.currentInverseQuestion) return;
      const { options, answer } = appStore.currentInverseQuestion;
      const eliminatable = options.filter(
        opt =>
          opt !== answer &&
          !appStore.wrongAnswerStrings.includes(opt) &&
          !appStore.autoEliminatedStrings.includes(opt)
      );
      if (eliminatable.length > 0) {
        const pick = eliminatable[Math.floor(Math.random() * eliminatable.length)];
        appStore.autoEliminateStringOption(pick);
      }
    }, 15000);
    return () => clearInterval(interval);
  }, [currentInverseQuestion?.id, showSuccess, quizComplete]);
    ...
    if (showSuccess) {
      ...
      successTimerRef.current = setTimeout(() => {
        if (appStore.quizComplete) {
          ...
          appStore.nextInverseQuestion(generateInverseQuestion(difficulty));
        }
      }, 2200);
    }
    return () => {
  ...
  }, [showSuccess, currentInverseQuestion?.id]);

  if (!currentInverseQuestion) return null;

  const { question, answer, options } = currentInverseQuestion;
  const accuracy =
  ...
  const handleAnswer = (option: string) => {
    if (showSuccess || wrongAnswerStrings.includes(option) || autoEliminatedStrings.includes(option))
      return;
    if (option === answer) appStore.markInverseCorrect();
    else appStore.markWrongAnswerString(option);
  };

  const handleSkip = () => {
    appStore.skipInverseQuestion(generateInverseQuestion(difficulty));
  };

  const basePoints = MISTAKE_POINTS[wrongAnswerStrings.length + autoEliminatedStrings.length] ?? 0;
  ...
  return (
    <div className="flex flex-col items-center gap-6 w-full max-w-2xl">
      ...
      {showSuccess && (
        <div className="fixed inset-0 flex flex-col items-center justify-center z-50 pointer-events-none">
          <div className="bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm rounded-3xl shadow-2xl p-10 flex flex-col items-center gap-3 border-4 border-green-400 dark:border-green-500 animate-bounce">
            ...
            <div className="text-xl text-gray-600 dark:text-gray-300 font-semibold text-center">
              {answer} = {question}! Great job! 🌟
            </div>
      ...
      {/* Question card */}
      <div className="w-full rounded-3xl shadow-2xl overflow-hidden border-4 border-white dark:border-gray-700 bg-teal-50 dark:bg-teal-950/60">
        <div className="bg-gradient-to-r from-teal-500 to-cyan-500 px-8 py-5 flex items-center gap-3">
          <span className="text-4xl">🔀</span>
          <span className="text-white font-bold text-xl opacity-90">Inverse Arithmetic</span>
        </div>
        ...
        <div className="px-8 py-10 flex flex-col items-center gap-8">
          <div className="text-center">
            ...
            <p className="text-8xl font-extrabold text-gray-800 dark:text-gray-100 tracking-wide">
              {question}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 w-full max-w-md">
            {options.map((option, idx) => {
              const isWrong = wrongAnswerStrings.includes(option);
              const isHint = autoEliminatedStrings.includes(option);
              return (
                <button
                  key={idx}
                  ...
                  className={`
                    relative flex items-center justify-center gap-3 px-4 py-5 rounded-2xl
                    ...
                    ${
                      isWrong
                        ...
                        : 'bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:scale-105 hover:shadow-lg hover:border-teal-300 dark:hover:border-teal-500 active:scale-95 cursor-pointer shadow-md'
                    }
                  `}
                >
                  <span
                    ...
                    ${
                      isWrong
                        ? 'bg-red-300 dark:bg-red-700 text-white'
                        : isHint
                  ...
                  <span className={`font-mono ${isWrong || isHint ? 'line-through' : ''}`}>
                    {option}
                  </span>
                  {isWrong && (
                  ...
                  {isHint && (
                    <span className="absolute top-1 right-2 text-xs font-bold text-gray-400 dark:text-gray-500">
                      hint
                    </span>
                  )}
                </button>
          ...
          <button
            ...
            className="flex items-center gap-2 text-gray-400 dark:text-gray-500 hover:text-teal-500 dark:hover:text-teal-400 text-base font-semibold transition-colors disabled:opacity-40 mt-2"
          >
...
export default InverseArithmeticCard;
